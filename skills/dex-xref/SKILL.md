...
# 默认就是 JSON（机器可读）；下面不再重复写 --format json
java -jar baksmali.jar xref callers app.apk --target "Lcom/Example;->foo()V"
...
默认就是 **JSON**（机器可读，面向 Agent / 脚本）。要人读文本加 `--format text`。
...
用仓库自带的 `accessorTest.dex` fixture（一个含合成访问器的测试 dex）。该 dex 中
`AccessorTypes;->access$072(...)` 是内部类调用的桥接方法：
...
可以看到 `boolean_and(Z)V` 在偏移 `0x2` 处调用了 `access$072`——这正是内部类访问外部类私有成员时编译器生成的合成调用。
