...
成功时默认输出一行 **JSON 报告**（机器可读，面向 Agent / 脚本）；要人读文本加 `--format text`。报告始终包含 `command`/`input`/`output` 三个公共字段，再加各命令特有的统计字段。
...
`matched` 表示被改写的方法数；`classFilter`/`methodFilter` 仅在显式给出时才出现在报告中。要人读文本，加 `--format text`（如 `Wrote /tmp/patched.dex (1 method(s) forced to return void).`）。
