...
baksmali fingerprint app.apk --format json        # 机器可读
```
...
baksmali fingerprint app.apk --match lib.dex --format json  # [{class, match, similarity}]
```
...
baksmali fingerprint appA.apk --format json | jq -r '.[].fingerprint' | sort > a.txt
baksmali fingerprint appB.apk --format json | jq -r '.[].fingerprint' | sort > b.txt
